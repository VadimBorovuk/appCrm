  ...
  <ul>
    <li v-for="(item, index) in menuItems" :key="index" class="mb-3 cursor-pointer">
      <div v-if="item.children">
        <NuxtLink
            active-class="text-waterloo-700 font-bold pointer"
            class="w-full flex items-center p-2 hover:bg-waterloo-50 rounded-md"
            :class="{ 'bg-waterloo-100 text-waterloo-700': isActiveParent(item) }"
        ...
        >
          ...
          <UIcon
              ...
              class="ml-auto text-waterloo-700"
          />
        </NuxtLink>
        ...
        <ul v-if="isOpen(index)" class="pl-4 mt-2">
          <li v-for="(child, childIndex) in item.children" :key="childIndex">
            <NuxtLink
                active-class="text-waterloo-900 font-bold"
                :to="child.to"
                class="w-full flex items-center py-1 pl-6 text-md rounded-md"
                :class="$route.path !== item.to ? 'hover:bg-waterloo-50' :''"
            >
              {{ child.label }}
            </NuxtLink>
          </li>
      ...
      <NuxtLink
          ...
          active-class="bg-waterloo-100 text-waterloo-700 rounded-md"
          class="w-full flex items-center p-2 rounded-md"
          :class="$route.path !== item.to ? 'hover:bg-waterloo-50' :''"
      >
        <Icon class="mr-2" :size="25" :name="item.icon" :class="{ 'text-waterloo-700': $route.path === item.to }"/>
        <client-only>
          {{ $t(item.label) }}
        </client-only>
      </NuxtLink>
